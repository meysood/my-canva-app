...
async function addFrameFromPaths(paths: string[], viewBox = { width: 1000, height: 1000, left: 0, top: 0 }) {
  ...
  await addElementAtPoint({
    type: "shape", top: box.top, left: box.left, width: box.width, height: box.height,
    paths: paths.map((d) => ({ d, fill: { dropTarget: true } })), viewBox,
  });
}
...
async function addFrameWithEffect(paths: string[], viewBox: { width: number; height: number; left: number; top: number }, effect: FrameEffect, effectColor: string) {
  ...
  try {
    if (effect === "double") {
      const s2 = box.width * 1.16; const o2 = (s2 - box.width) / 2;
      await addElementAtPoint({ type: "shape", top: box.top - o2, left: box.left - o2, width: s2, height: s2, paths: paths.map((d) => ({ d, fill: { color: effectColor } })), viewBox });
      await delay(250);
      const s1 = box.width * 1.08; const o1 = (s1 - box.width) / 2;
      await addElementAtPoint({ type: "shape", top: box.top - o1, left: box.left - o1, width: s1, height: s1, paths: paths.map((d) => ({ d, fill: { color: "#FFFFFF" } })), viewBox });
      await delay(250);
    }
    if (effect === "outline") {
      const s1 = box.width * 1.10; const o1 = (s1 - box.width) / 2;
      await addElementAtPoint({ type: "shape", top: box.top - o1, left: box.left - o1, width: s1, height: s1, paths: paths.map((d) => ({ d, fill: { color: effectColor } })), viewBox });
      await delay(250);
    }
    if (effect === "glow") {
      const s1 = box.width * 1.12; const o1 = (s1 - box.width) / 2;
      await addElementAtPoint({ type: "shape", top: box.top - o1, left: box.left - o1, width: s1, height: s1, paths: paths.map((d) => ({ d, fill: { color: effectColor } })), viewBox });
      await delay(250);
    }
    if (effect === "shadow") {
      await addElementAtPoint({ type: "shape", top: box.top + 8, left: box.left + 8, width: box.width, height: box.height, paths: paths.map((d) => ({ d, fill: { color: "#333333" } })), viewBox });
      await delay(250);
    }
...
function getSavedFrames(): SavedFrame[] { try { const r = localStorage.getItem("frame_maker_library"); return r ? JSON.parse(r) : []; } catch { return []; } }
...
function deleteFrame(id: string) { const frames = getSavedFrames().filter((f) => f.id !== id); localStorage.setItem("frame_maker_library", JSON.stringify(frames)); }

function Spinner({ size = 16 }: { size?: number }) {
  return <div style={{ width: size, height: size, border: "2px solid rgba(255,255,255,0.3)", borderTop: "2px solid white", borderRadius: "50%", animation: "spin 0.6s linear infinite", display: "inline-block", marginRight: 6 }} />;
}
...
function Confetti() {
  return (
    <div style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", pointerEvents: "none", zIndex: 9999 }}>
      {[...Array(30)].map((_, i) => (
        <div key={i} style={{ position: "absolute", width: 8, height: 8, background: ["#667eea", "#764ba2", "#f093fb", "#4facfe", "#43e97b", "#fa709a"][i % 6], left: `${Math.random() * 100}%`, top: "-10px", borderRadius: Math.random() > 0.5 ? "50%" : "2px", animation: `fall ${1 + Math.random() * 2}s linear`, animationDelay: `${Math.random() * 0.5}s` }} />
      ))}
    </div>
  );
}
...
function App() {
  ...
  // Watch text selection from canvas
  useEffect(() => {
    try {
  ...
  const onAddPreset = async (preset: Preset) => {
    try { setStatus("Adding frame…"); const f = FRAME_PATHS[preset]; await addFrameFromPaths(f.paths, f.viewBox); showSuccess("✅ Frame added!"); }
  ...
  const onFileSelect = (file: File | null) => {
    ...
    setSelectedFile(file); setPreviewUrl(URL.createObjectURL(file)); setStatus(""); setShowTutorial(false);
  };
  ...
  const onConvertToFrame = async () => {
    ...
    try {
      ...
      const txt = await resp.text(); let data: any;
      ...
      setTimeout(() => clearFileSelection(), 2500);
    } catch (e: any) { setStatus(`❌ ${e?.message || "Failed"}`); }
  ...
  const onBatchConvert = async () => {
    ...
    try {
      ...
      const form = new FormData(); batchFiles.forEach((f) => form.append("files", f));
      ...
      try { data = JSON.parse(txt); } catch { throw new Error(`Backend error (${resp.status})`); }
      if (!resp.ok) throw new Error(data?.error || `Failed (${resp.status})`);
      const results = data.results || []; let added = 0;
      for (let i = 0; i < results.length; i++) {
        const r = results[i]; setBatchProgress(Math.round(((i + 1) / results.length) * 100));
        setStatus(`Frame ${i + 1}/${results.length}…`);
        if (r.paths?.length) { await addFrameFromPaths(r.paths, r.viewBox); saveFrame({ name: r.name || `batch-${i}`, paths: r.paths, viewBox: r.viewBox }); added++; await delay(300); }
      }
      setSavedFrames(getSavedFrames());
  ...
  const onAddFromLibrary = async (frame: SavedFrame) => {
    try { setStatus("Adding…"); await addFrameFromPaths(frame.paths, frame.viewBox); showSuccess("✅ Added from library!"); }
    catch (e: any) { setStatus(`❌ ${e?.message || "Failed"}`); }
  };

  const onAddWithEffect = async () => {
    try {
      setIsConverting(true); setStatus(`Adding ${selectedFrameForEffect} with ${selectedEffect}…`);
    ...
    } catch (e: any) { setStatus(`❌ ${e?.message || "Effect failed"}`); }
    finally { setIsConverting(false); }
  };
  ...
  const onConvertTextToFrame = async () => {
    ...
    try {
      ...
      setStatus("🔄 Converting text to frame…");

      const resp = await fetch(`${BACKEND_URL.replace(/\/$/, "")}/text-to-frame`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ text, fontSize: textFontSize, fontWeight: "bold" }),
      });
      ...
      if (!paths.length) throw new Error("Could not convert text to frame");

      setStatus("Creating frame…");
      await addFrameFromPaths(paths, data.viewBox);

      saveFrame({ name: `Text: ${text.substring(0, 20)}`, paths, viewBox: data.viewBox });
      setSavedFrames(getSavedFrames());

      showSuccess("🎉 Text converted to frame!");
    } catch (e: any) {
  ...
  return (
    <div ref={dropZoneRef} style={{ padding: 0, fontFamily: "system-ui, -apple-system, Segoe UI, Roboto", minHeight: "100vh" }}>
      ...
      <div style={{ padding: 16, animation: "slideIn 0.2s ease-out" }}>
          ...
          <div>
            {Object.entries(FRAME_PACKS).map(([key, pack]) => (
              <div key={key} style={{ marginBottom: 16 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 6 }}>
                  {pack.frames.map((frame, i) => <button key={i} className="frame-grid-btn" onClick={() => onAddPreset(frame)}>{frame.charAt(0).toUpperCase() + frame.slice(1)}</button>)}
                </div>
              </div>
            ))}
            <div style={{ marginTop: 8 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6 }}>
                {Object.keys(FRAME_PATHS).map((key) => <button key={key} className="frame-grid-btn" onClick={() => onAddPreset(key as Preset)}>{key.charAt(0).toUpperCase() + key.slice(1)}</button>)}
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === "custom" && (
          <div>
            ...
            <label htmlFor="file-input" style={{ display: "block", padding: "14px 16px", background: isDragging ? "rgba(102,126,234,0.1)" : "#F5F5F5", color: "#333", borderRadius: 8, border: `2px dashed ${isDragging ? "#667eea" : "#CCC"}`, cursor: isConverting ? "not-allowed" : "pointer", fontSize: 13, fontWeight: 600, textAlign: "center", opacity: isConverting ? 0.5 : 1 }}>
              {isDragging ? "📂 Drop PNG here" : selectedFile ? `✓ ${selectedFile.name}` : "📁 Choose or Drop PNG"}
            </label>
            <div style={{ marginTop: 12, display: "flex", gap: 8, flexWrap: "wrap" }}>
              <label style={{ fontSize: 12, display: "flex", alignItems: "center", gap: 4, cursor: "pointer" }}><input type="checkbox" checked={useSmartCrop} onChange={(e) => setUseSmartCrop(e.target.checked)} /> Smart Crop</label>
              <label style={{ fontSize: 12, display: "flex", alignItems: "center", gap: 4, cursor: "pointer" }}><input type="checkbox" checked={useRemoveBg} onChange={(e) => setUseRemoveBg(e.target.checked)} /> Remove BG</label>
            </div>
            {previewUrl && (
              <div style={{ marginTop: 12 }}>
                <div style={{ position: "relative", width: "100%" }}>
                  <img src={previewUrl} alt="preview" style={{ width: "100%", maxHeight: 180, objectFit: "contain", borderRadius: 8, border: "2px solid #E5E5E5", background: "repeating-conic-gradient(#E5E5E5 0% 25%, white 0% 50%) 50% / 20px 20px", opacity: isConverting ? 0.6 : 1 }} />
                  <button onClick={clearFileSelection} disabled={isConverting} style={{ position: "absolute", top: 8, right: 8, width: 28, height: 28, borderRadius: "50%", background: "rgba(0,0,0,0.7)", border: "2px solid white", color: "white", fontSize: 16, cursor: isConverting ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", padding: 0, lineHeight: 1 }} title="Remove (Esc)">✕</button>
                </div>
                <button style={{ marginTop: 10, width: "100%", padding: "12px 16px", background: isConverting ? "#999" : "linear-gradient(135deg, #667eea 0%, #764ba2 100%)", color: "white", borderRadius: 8, border: "none", cursor: isConverting ? "not-allowed" : "pointer", fontSize: 14, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={onConvertToFrame} disabled={!selectedFile || isConverting}>
        ...
        {activeTab === "batch" && (
          <div>
            ...
            <label htmlFor="batch-input" style={{ display: "block", padding: "14px 16px", background: "#F5F5F5", color: "#333", borderRadius: 8, border: "2px dashed #CCC", cursor: "pointer", fontSize: 13, fontWeight: 600, textAlign: "center" }}>
            ...
            {batchFiles.length > 0 && (
              <div style={{ marginTop: 12 }}>
                <div style={{ maxHeight: 150, overflowY: "auto", marginBottom: 10 }}>
                  {batchFiles.map((f, i) => <div key={i} style={{ fontSize: 11, padding: "4px 0", color: "#666", display: "flex", justifyContent: "space-between" }}><span>{f.name}</span><span style={{ opacity: 0.5 }}>{(f.size / 1024).toFixed(0)}KB</span></div>)}
                </div>
                {batchProgress > 0 && <div style={{ height: 6, background: "#E5E5E5", borderRadius: 3, marginBottom: 10, overflow: "hidden" }}><div style={{ height: "100%", width: `${batchProgress}%`, background: "linear-gradient(90deg, #667eea, #764ba2)", borderRadius: 3, transition: "width 0.3s" }} /></div>}
                <div style={{ display: "flex", gap: 8 }}>
                  <button style={{ flex: 1, padding: "12px", background: "linear-gradient(135deg, #667eea 0%, #764ba2 100%)", color: "white", borderRadius: 8, border: "none", cursor: isConverting ? "not-allowed" : "pointer", fontSize: 13, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={onBatchConvert} disabled={isConverting}>
                    {isConverting ? <><Spinner /> Converting...</> : `Convert All (${batchFiles.length})`}
                  </button>
                  <button style={{ padding: "12px 16px", background: "#F5F5F5", color: "#666", borderRadius: 8, border: "none", cursor: "pointer", fontSize: 13, fontWeight: 600 }} onClick={() => setBatchFiles([])}>Clear</button>
                </div>
              </div>
            )}
        ...
        {activeTab === "library" && (
          <div>
            {savedFrames.length === 0 ? (
              <div style={{ textAlign: "center", padding: 30, color: "#999" }}><div style={{ fontSize: 32, marginBottom: 8 }}>📚</div><div style={{ fontSize: 13 }}>No saved frames yet</div><div style={{ fontSize: 11, marginTop: 4 }}>Convert a PNG to start your library</div></div>
            ) : (
              <div>
                <div style={{ fontSize: 11, marginBottom: 10, color: "#888" }}>{savedFrames.length} saved</div>
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {savedFrames.map((frame) => (
                    <div key={frame.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 10px", background: "#F9F9F9", borderRadius: 8 }}>
                      <div><div style={{ fontSize: 13, fontWeight: 600 }}>{frame.name}</div><div style={{ fontSize: 10, color: "#999" }}>{frame.paths.length} paths • {new Date(frame.timestamp).toLocaleDateString()}</div></div>
                      <div style={{ display: "flex", gap: 6 }}>
                        <button onClick={() => onAddFromLibrary(frame)} style={{ padding: "6px 12px", background: "#7D2AE8", color: "white", borderRadius: 6, border: "none", cursor: "pointer", fontSize: 11, fontWeight: 600 }}>Add</button>
                        <button onClick={() => { deleteFrame(frame.id); setSavedFrames(getSavedFrames()); }} style={{ padding: "6px 10px", background: "#FEE", color: "#C33", borderRadius: 6, border: "none", cursor: "pointer", fontSize: 11, fontWeight: 600 }}>✕</button>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {activeTab === "effects" && (
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 6, marginBottom: 16 }}>
              {(Object.keys(FRAME_PATHS) as Preset[]).map((key) => <button key={key} onClick={() => setSelectedFrameForEffect(key)} style={{ padding: "8px 4px", background: selectedFrameForEffect === key ? "#7D2AE8" : "#F5F5F5", color: selectedFrameForEffect === key ? "white" : "#333", borderRadius: 6, border: "none", cursor: "pointer", fontSize: 11, fontWeight: 600 }}>{key.charAt(0).toUpperCase() + key.slice(1)}</button>)}
            </div>
            <div style={{ fontSize: 12, marginBottom: 8, fontWeight: 600, color: "#555" }}>Choose Effect</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, marginBottom: 16 }}>
              {(["none", "shadow", "glow", "outline", "double"] as FrameEffect[]).map((effect) => <button key={effect} onClick={() => setSelectedEffect(effect)} style={{ padding: "10px", background: selectedEffect === effect ? "#667eea" : "#F5F5F5", color: selectedEffect === effect ? "white" : "#333", borderRadius: 8, border: "none", cursor: "pointer", fontSize: 12, fontWeight: 600 }}>{effect === "none" ? "No Effect" : effect.charAt(0).toUpperCase() + effect.slice(1)}</button>)}
            </div>
            {selectedEffect !== "none" && (
              <>
                <div style={{ fontSize: 12, marginBottom: 8, fontWeight: 600, color: "#555" }}>Effect Color</div>
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 8 }}>
                  {COLOR_PRESETS.map((c) => <button key={c} onClick={() => { setSelectedColor(c); setCustomColor(c); }} style={{ width: 28, height: 28, borderRadius: "50%", background: c, border: selectedColor === c ? "3px solid #333" : "2px solid #DDD", cursor: "pointer", padding: 0 }} />)}
                </div>
                <div style={{ display: "flex", gap: 6, alignItems: "center", marginBottom: 16 }}>
                  <input type="color" value={customColor} onChange={(e) => { setCustomColor(e.target.value); setSelectedColor(e.target.value); }} style={{ width: 32, height: 32, border: "none", cursor: "pointer", padding: 0 }} />
                  <span style={{ fontSize: 11, color: "#888" }}>Custom color</span>
                </div>
              </>
            )}
            <div style={{ padding: "10px 12px", background: "#F9F9F9", borderRadius: 8, marginBottom: 12, fontSize: 12, color: "#666" }}>
              <strong>{selectedFrameForEffect}</strong>
              {selectedEffect !== "none" && <> + <strong>{selectedEffect}</strong> effect<span style={{ display: "inline-block", width: 12, height: 12, borderRadius: "50%", background: selectedColor, marginLeft: 6, verticalAlign: "middle", border: "1px solid #DDD" }} /></>}
            </div>
            <button onClick={onAddWithEffect} disabled={isConverting} style={{ width: "100%", padding: "12px 16px", background: isConverting ? "#999" : "linear-gradient(135deg, #667eea 0%, #764ba2 100%)", color: "white", borderRadius: 8, border: "none", cursor: isConverting ? "not-allowed" : "pointer", fontSize: 14, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center" }}>
              {isConverting ? <><Spinner /> Adding...</> : `Add Frame${selectedEffect !== "none" ? ` + ${selectedEffect}` : ""}`}
            </button>
            <div style={{ fontSize: 11, marginTop: 10, opacity: 0.5 }}>Effects add multiple layers — ungroup to customize</div>
          </div>
        )}

        {/* SELECTION TAB — TEXT/SHAPE TO FRAME */}
        {activeTab === "selection" && (
          <div>
            <div style={{ display: "inline-block", padding: "3px 10px", background: "linear-gradient(135deg, #f093fb 0%, #f5576c 100%)", color: "white", borderRadius: 12, fontSize: 10, fontWeight: 700, marginBottom: 14, letterSpacing: 0.5 }}>
              ✦ PRO FEATURE
            </div>

            <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 6, color: "#333" }}>Text / Shape → Frame</div>
            <div style={{ fontSize: 12, color: "#666", marginBottom: 16, lineHeight: 1.5 }}>
              Convert any <strong>text</strong> into a frame shape. Select text on canvas or type below.
            </div>

            {/* Auto-detect from canvas */}
            {hasTextSelection && (
              <div style={{ padding: "10px 12px", background: "#E8F5E9", border: "1px solid #A5D6A7", borderRadius: 8, marginBottom: 12 }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: "#2E7D32", marginBottom: 4 }}>✅ Text detected from canvas:</div>
                <div style={{ fontSize: 14, fontWeight: 700, color: "#1B5E20" }}>"{selectedText}"</div>
              </div>
            )}

            {/* Manual text input */}
            <div style={{ fontSize: 12, marginBottom: 6, fontWeight: 600, color: "#555" }}>Enter text to convert</div>
            <input
              type="text"
              value={manualText}
              ...
              maxLength={50}
              style={{
                width: "100%", padding: "12px 14px", borderRadius: 8, border: "2px solid #DDD",
            ...
            {/* Font size slider */}
            <div style={{ marginTop: 12 }}>
              <div style={{ fontSize: 12, marginBottom: 4, fontWeight: 600, color: "#555", display: "flex", justifyContent: "space-between" }}>
                <span>Detail Level</span>
                <span style={{ color: "#888" }}>{textFontSize}</span>
              </div>
              <input
                type="range"
                min={80}
            ...
            {manualText.trim() && (
              <div style={{
                marginTop: 14, padding: "20px", background: "#F9F9F9", borderRadius: 8,
                textAlign: "center", border: "1px solid #E5E5E5",
              }}>
                <div style={{
                  fontSize: Math.min(textFontSize / 5, 48), fontWeight: 700, color: "#333",
                  wordBreak: "break-word",
                }}>
                  {manualText}
                </div>
                <div style={{ fontSize: 10, color: "#999", marginTop: 6 }}>Preview — actual frame will be vectorized</div>
              </div>
            )}

            {/* Convert button */}
            <button
              onClick={onConvertTextToFrame}
              disabled={!manualText.trim() || isConverting}
              style={{
                marginTop: 14, width: "100%", padding: "14px 16px",
                background: !manualText.trim() || isConverting ? "#999" : "linear-gradient(135deg, #f093fb 0%, #f5576c 100%)",
                color: "white", borderRadius: 10, border: "none",
                cursor: !manualText.trim() || isConverting ? "not-allowed" : "pointer",
                fontSize: 15, fontWeight: 700,
                display: "flex", alignItems: "center", justifyContent: "center",
                boxShadow: !manualText.trim() || isConverting ? "none" : "0 4px 15px rgba(240,147,251,0.4)",
              }}
            >
              {isConverting ? <><Spinner /> Converting...</> : "✦ Convert to Frame"}
            </button>
            ...
            <div style={{ fontSize: 11, marginTop: 12, opacity: 0.5, textAlign: "center" }}>
              Works best with bold, short text (1-3 words)
            </div>
          </div>
        )}
      ...
      {isDragging && activeTab !== "custom" && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(102,126,234,0.15)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }}>
          <div style={{ background: "white", padding: "20px 30px", borderRadius: 12, boxShadow: "0 4px 20px rgba(0,0,0,0.15)", fontSize: 16, fontWeight: 700, color: "#667eea" }}>📂 Drop PNG here</div>
        </div>
      )}
    </div>
  );
